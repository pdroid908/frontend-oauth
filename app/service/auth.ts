...
export const forgotPassword = async (email: string) => {
  ...
  const response = await fetch("https://pasdaoji-backend-oauth.hf.space/forgot-password", {
    method: "POST",
...
export const resetPassword = async (token: string, new_password: string) => {
  ...
  const response = await fetch("https://pasdaoji-backend-oauth.hf.space/reset-password", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
